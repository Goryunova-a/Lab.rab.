...
- Данная программа показывает как работает класс с инкапсуляцией. Метод класса, который служит конструктором, должен называться init. Здесь он назван просто init, что является ошибкой.
...
class Tomato:
    ...
    def __init__(self, index):#метод с динамическими свойствами _index и _state
        self._index = index
        self._state = 0
...
class TomatoBush:
    def __init__(self, num):#метод, который принимает в качестве параметра кол-во томатов и создёт список объектов класса.
        #список хранится внутри динамического свойства tomatoes
...
class Gardener:
    def __init__(self, name, plant):#метод с динамическими свойствами
        self.name = name
        self._plant = plant

    def work(self):#метод, позвояющий садовнику работать, а томату расти
        if self._plant.tomatoes:
            self._plant.grow_all()

    def harvest(self):# метод прооверяет зрелость, если не созрели, будет предупреждение
...
#собираем урожай
```
![Меню](https://github.com/Goryunova-a/Lab.rab./blob/main/pic9/pic6.png)
